    ...
    public static final String HTML = """
        ...
        <style>
          ...
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          /* ===== 返回主页（右上角文字链） ===== */
          .home-btn {
            position: absolute; right: 20px; top: 22px; z-index: 5;
            ...
            transition: all .25s; border: 1px solid rgba(255,255,255,.3);
            letter-spacing: .5px;
          }
          .home-btn:hover { background: rgba(255,255,255,.36); transform: translateY(-1px); }
          ...
          .footer .text a { color: var(--muted); text-decoration: none; border-bottom: 1px dashed var(--muted); }
          .footer .text a:hover { color: var(--primary); border-color: var(--primary); }
          ...
          @media (max-width: 640px) {
            ...
            .home-btn { right: 12px; top: 12px; font-size: 12px; padding: 5px 12px; }
        ...
        <!-- 页头 -->
        <div class="hero">
          <a class="home-btn" href="/" title="返回博客主页">返回主页 &rsaquo;</a>
          <div class="emoji">&#127778;</div>
          <h1>开心一刻</h1>
          ...
          <div id="newsApp">
            <div class="toolbar">
              ...
              <div class="actions">
                ...
                <button class="btn primary" id="syncBtn"><span class="spin">&#128229;</span> 获取新的库存数据</button>
              </div>
            </div>
        ...
        <div class="footer">
          <div class="wave"></div>
          <div class="text">
            数据来源：内置笑话集 &middot; 一言（hitokoto）句子库 &middot; <a href="/">返回主页</a><br>
            Made with <span class="heart">&#10084;</span> for 打工人 &middot; 仅作放松使用，请勿用于商业用途
          </div>
        </div>
        ...
        (function () {
          ...
          function render() {
            ...
            if (!pool.length) {
              currentBatch = [];
              listEl.innerHTML = '<div class="empty"><span class="icon">🐛</span>暂无内容，点击〈获取新的库存数据〉试试</div>';
              return;
            }
          ...
          /* 获取新的库存数据：先尝试服务端同步（管理员生效），
             再重新拉取库存并重抽；匿名 403 自动降级为重新读取 */
          function syncNow() {
            ...
            fetch(BASE + '/hotspot-news/refresh', { method: 'POST' })
              ...
              .then(function (synced) {
                return fetch(BASE + '/hotspot-news').then(function (r2) {
                  ...
                  return r2.json().then(function (data) { return { synced: synced, data: data }; });
                });
              })
              .then(function (res) {
                if (res && res.data) {
                  ...
                  updateStats();
                  swapBatch();
                  showToast(res.synced ? '已获取新的库存数据 ✨' : '已重新读取库存数据');
                } else {
                  showToast('暂时无法获取库存数据，请稍后再试');
                }
              })
              .catch(function () { showToast('获取库存数据失败'); })
              .finally(function () {
                setTimeout(function () {
                  btn.disabled = false;
                  btn.classList.remove('loading');
